 ...
 */
    private boolean validatePassword(String password){//Ja1234*
        ...
        String lowerLetters=password.replaceAll("[^a_z]","");
        String num=password.replaceAll("[^0-9]","");
        String symbol=password.replaceAll("[^!@#$%^&*()_+{}\\[\\]:;<>,.?/~\\\\|]","");
        boolean space=password.contains(" ");
        boolean lengthGtSix=password.length()>5;
        boolean existsUpperLetter=upperLetters.length()>0;
        boolean existsLowerLetter=lowerLetters.length()>0;
        boolean existNum=num.length()>0;
